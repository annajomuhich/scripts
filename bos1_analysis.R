...
lesion <- lesion %>%
	filter(Plant_geno_shorthand == "Bos")
means <- lesion %>%
...
ggplot(data = pca_df, aes(PC1,
	...
	labs(color = "Mean Lesion Size mm2",
			 title = "PCA of normalized Bcin reads from Bos_1")
...
bcin_forpca_cat <- bcin_forpca_cat %>%
	filter(sample_ID != "B55_1") %>%
	filter(sample_ID != "B51_1") %>%
	filter(sample_ID != "B44_1") %>%
	filter(sample_ID != "B67_1")
#remove categorical variables
bcin_forpca <- bcin_forpca_cat[,c(10:11173)]
#put in gene rownames
row.names(bcin_forpca) <- bcin_forpca_cat$sample_ID
...
ggplot(data = pca_df, aes(PC1,
	...
	theme_minimal() +
	labs(color = "Mean Lesion Size mm2",
			 title = "PCA of normalized Bcin reads from Bos_1 - no outliers")
...
ggplot(data = pca_df, aes(PC1,
	...
	theme_minimal() +
	labs(color = "Mean Lesion Size mm2",
			 title = "PCA of normalized Host reads from Bos_1")
